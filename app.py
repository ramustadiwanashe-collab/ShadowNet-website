...
from content_notes import make_note
...
def language(slug):
    if slug not in LANGUAGES:
        abort(404)
    notes = {level: [make_note(slug, topic, level) for topic in TOPICS[slug][level]] for level in TOPICS[slug]}
    return render_template("language.html", language=LANGUAGES[slug], slug=slug, topic_data=TOPICS[slug], notes=notes)
...
def call_openai(message, username):
    if not OPENAI_API_KEY:
        return None, "OPENAI_API_KEY is missing from the server environment."
    ...
    for model in models:
        ...
        try:
            ...
            last_error = "The AI service returned no text."
        except urllib.error.HTTPError as exc:
            try:
                details = json.loads(exc.read().decode("utf-8"))
                last_error = details.get("error", {}).get("message", f"OpenAI HTTP {exc.code}")
            except Exception:
                last_error = f"OpenAI HTTP {exc.code}"
            if exc.code in (401, 403):
                break
        except urllib.error.URLError as exc:
            last_error = f"Network error contacting AI service: {exc.reason}"
            break
        except (TimeoutError, json.JSONDecodeError) as exc:
            last_error = f"AI response error: {exc}"
    return None, last_error


def local_shadowbot_reply(message, language=None):
    lower = message.lower()
    if "explain" in lower or "what is" in lower or "how does" in lower:
        topic = language or "programming"
        return f"I can explain {topic} step by step. Start with the definition, then a small example, then test an edge case. Ask me about a specific concept and I will break it down."
    if "error" in lower or "bug" in lower or "debug" in lower:
        return "For debugging: reproduce the problem, capture the exact error, isolate the smallest failing example, inspect inputs and state, then fix and retest. Send me the error message and the relevant code."
    return "ShadowBot is ready in local study mode. Ask a specific question such as 'Explain Python functions', 'How do SQL joins work?', or 'Debug this JavaScript error'."
...
def assistant():
    ...
    user = current_user()
    reply, error = call_openai(message, user["username"]) if user else (None, "Not signed in")
    if reply:
        return jsonify({"reply": reply, "model": OPENAI_MODEL, "configured": True})

    # Keep the assistant useful even while an API key/model is being configured.
    return jsonify({
        "reply": local_shadowbot_reply(message),
        "model": OPENAI_MODEL,
